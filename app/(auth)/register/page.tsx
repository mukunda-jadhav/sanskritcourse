    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
      <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl p-8 border border-orange-100 dark:border-gray-800">
        <div className="text-center mb-8">
          ...
          <h1 className="font-playfair text-2xl font-bold text-gray-900 dark:text-white">Join Sanskrit Gurukul</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Begin your Sanskrit learning journey today</p>
        </div>

        <button onClick={async () => { setGoogleLoading(true); await signIn('google', { callbackUrl: '/dashboard' }); }}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <Input label="Full Name" placeholder="Arjun Sharma" icon={<User className="h-4 w-4" />} error={errors.name?.message} {...register('name')} />
          ...
          <Input label="Confirm Password" type="password" placeholder="Repeat password" icon={<Lock className="h-4 w-4" />} error={errors.confirmPassword?.message} {...register('confirmPassword')} />
          <Button type="submit" className="w-full" loading={isSubmitting}>Create Account</Button>
        </form>

        <p className="text-center text-sm text-gray-500 dark:text-gray-400 mt-6">
          Already have an account?{' '}
          <Link href="/login" className="text-orange-500 font-medium hover:text-orange-600">Sign in</Link>
        </p>
      </div>
    </motion.div>
  );
